...
public class SettingsActivity extends AppCompatActivity {
    ...
    private void loadCurrentSettings() {
        ...
        if (theme.equals("light") || theme.equals("white")) {
            themeRadioGroup.check(R.id.theme_light);
        } else if (theme.equals("dark") || theme.equals("black")) {
    ...
    private void applyThemeFromDatabase() {
        ...
        if (user != null) {
            ...
            Log.d("SettingsTheme", "Applying theme: " + theme);
            applyThemeColors(theme);
        } else {
            String theme = preferences.getString("color_theme", "light");
            Log.d("SettingsTheme", "No user, applying theme from prefs: " + theme);
            applyThemeColors(theme);
        }
    }

    // NEW METHOD: Set CardView backgrounds
    private void setCardViewBackgrounds(int color) {
        if (mainLayout != null && mainLayout instanceof ViewGroup) {
            findAndSetCardViewBackground((ViewGroup) mainLayout, color);
        }
    }

    // NEW METHOD: Recursively find and set CardView backgrounds
    private void findAndSetCardViewBackground(ViewGroup viewGroup, int color) {
        for (int i = 0; i < viewGroup.getChildCount(); i++) {
            ...
            if (child instanceof ViewGroup) {
                findAndSetCardViewBackground((ViewGroup) child, color);
            }
        }
    }

    // UPDATED METHOD: Now includes CardView background handling
    private void applyThemeColors(String theme) {
        ...
        int backgroundColor;
        int textColor;
        int cardBackgroundColor; // NEW: Card background color
        ...
        if (isDarkTheme) {
            ...
            cardBackgroundColor = ContextCompat.getColor(this, R.color.color_card_dark); // NEW
        } else {
            // Light theme colors
            ...
            cardBackgroundColor = ContextCompat.getColor(this, R.color.color_card_light); // NEW
        }

        Log.d("SettingsTheme", "Theme: " + (isDarkTheme ? "DARK" : "LIGHT"));
        ...
        // NEW: Set CardView backgrounds
        setCardViewBackgrounds(cardBackgroundColor);

        // Set text colors for labels
        setTextColorForAllTextViews(textColor);
        ...
        // NEW: Set cancel button color
        if (cancelButton != null) {
            cancelButton.setBackgroundColor(ContextCompat.getColor(this, R.color.color_button_secondary));
            cancelButton.setTextColor(Color.WHITE);
        }

        if (notificationTimeButton != null) {
            notificationTimeButton.setBackgroundColor(primaryColor);
        ...
        // Bottom navigation icons stay white
        int navIconColor = Color.WHITE;
    ...
    private void setTextColorForAllTextViews(int textColor) {
        // Frequency text
        if (frequencyValueText != null) frequencyValueText.setTextColor(textColor);

        // Notification time text
        if (notificationTimeText != null) notificationTimeText.setTextColor(textColor);

        // Radio button texts
        setRadioGroupTextColor(vibrationRadioGroup, textColor);
        setRadioGroupTextColor(themeRadioGroup, textColor);
    }

    private void setRadioGroupTextColor(RadioGroup radioGroup, int color) {
        if (radioGroup == null) return;

        for (int i = 0; i < radioGroup.getChildCount(); i++) {
            View child = radioGroup.getChildAt(i);
            if (child instanceof RadioGroup) {
                setRadioGroupTextColor((RadioGroup) child, color);
    ...
    private void saveSettings() {
        ...
        String theme;
        if (selectedThemeId == R.id.theme_dark) {
            theme = "dark";
        } else {
            theme = "light";
        }
